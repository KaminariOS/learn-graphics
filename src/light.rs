...
pub struct LightRenderGroup {
    ...
    light_render_triplets: Vec<(Buffer, BindGroup, GeoObj)>
}



impl LightRenderGroup {
    pub fn new(device: &Device, light_uniforms_and_objs: Vec<(LightUniform, GeoObj)>, camera: &Camera, config: &SurfaceConfiguration) -> Rc<RefCell<Self>> {
        let (light_uniforms, objs): (Vec<LightUniform>, Vec<GeoObj>) = light_uniforms_and_objs.into_iter().unzip();
        ...
        let light_render_triplets: Vec<_> = light_uniforms.iter().zip(objs).map (|(light_uniform, obj)|{
            let buffer_per_light = device.create_buffer_init(
                &wgpu::util::BufferInitDescriptor {
                    label: Some("Light VB"),
    ...
    pub fn update_light(&mut self, dt: Duration, queue: &wgpu::Queue, state: &State) {
        let rotation: cgmath::Matrix3<f32> = cgmath::Quaternion::from_axis_angle(cgmath::Vector3::unit_y(), cgmath::Deg(-100. * dt.as_secs_f32())).into();
